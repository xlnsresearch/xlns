...
opt_mul = xltorch.optim.LNSSignMul(model_mul.lns_parameters(), lr=0.05, use_pow=False)
opt_mul_pow = xltorch.optim.LNSSignMul(model_mul_pow.lns_parameters(), lr=0.05, use_pow=True)
...
for step in range(1, n_steps + 1):
    ...
    print(f"Iter {step:02d} | loss_SGD={loss_sgd.item():.2e} | "
          f"loss_SignMul={loss_mul.item():.2e} | "
          f"loss_SignMul_pow={loss_mul_pow.item():.2e}")

plt.figure(figsize=(6, 4))
plt.plot(range(1, n_steps + 1), loss_hist_sgd, label='LNSSGD (lr=0.1)',  marker='o')
plt.plot(range(1, n_steps + 1), loss_hist_mul, label='LNSSignMul (lr=0.1)', marker='s')
plt.plot(range(1, n_steps + 1), loss_hist_mul_pow, label='LNSSignMul (lr=0.1, pow)', marker='^')
