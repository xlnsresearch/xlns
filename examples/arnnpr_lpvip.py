...
# arnnpr_lpvip <- arnabs8xlnsnp2024ruf.py <- arnabs8xlnsnp2024r.py  
# dot prod with redundant LNS via np lpvip (sb only)
###################################################
...
def sb_ufunc_premit(zi,s):   #was called premitchnpi(zi):
  ...
  #return ( (((1<<xlnsF)+(z&((1<<xlnsF)-1)))>>(-(z>>xlnsF)))+postcond )<<1
def dot(a,b):
     ar = xl.xlnsnpr(a)
     xl.sbdb_ufunc = sb_ufunc_premit
     rr = ar @ b
...
def main(main_params):
	...
	#XlnsExactsb = True 
	print(" # dot prod with redundant LNS via np lpvip (sb only)")
	#XlnsExactdb = True
	...
	if is_training:
		...
		for epoch in range(num_epoch):
			...
			for mbatch in range(int(split / batchsize)):
				...
				lnss1 = dot(xl.xlnsnp.hstack((lnsones, lnsx)), lnsW1)
				...
				lnss2 = dot(xl.xlnsnp.hstack((lnsones, lnsa1)), lnsW2)
				...
				lnsgrad_a1 = dot(lnsgrad_s2, xl.xlnsnp.transpose(lnsW2[1:]))
		...
		plt.suptitle('FP versus LNS xlnsnpr_lpvip '+str(split)+' Validation and Training MNIST Accuracies F='+str(xl.xlnsF), fontsize=14)
		ax.legend(['FP train', 'FP validation','LNS train', 'LNS validation'])
		plt.grid(which='both', axis='both', linestyle='-.')

		plt.savefig('plotmnist_npr_lpvip.png')

if __name__ == '__main__':

	parser = argparse.ArgumentParser()
	parser.add_argument('--is_training', default = True)
	parser.add_argument('--split', default = 50) #5000)
	parser.add_argument('--learning_rate', default = 0.01)
	parser.add_argument('--lambda', default = 0.000)     #.001
	parser.add_argument('--minibatch_size', default = 1) #5
	parser.add_argument('--num_epoch', default = 5) #12)  #40)     #10 20
