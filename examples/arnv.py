...
# arnv.py <- arnabs8xlnsvAiNvGd2024.py <- arnabs8xlnsudAiNvGd2024.py
# slow scalar "variable" precision LNS
###################################################
...
def main(main_params):
	print('  slow scalar "variable" precision LNS')
	...
	if is_training:
		...
		lnsW1 = np.array(xl.xlnscopy(list(W1),xl.xlnsv,setFB=6))
		lnsW2 = np.array(xl.xlnscopy(list(W2),xl.xlnsv,setFB=6))
		print(repr(lnsW1[0][0]))
		...
		print(repr(lnsdelta_W1[0,0]))
		print(repr(lnsdelta_W2[0,0]))
		...
		for epoch in range(num_epoch):
			...
			for mbatch in range(int(split / batchsize)):
				...
				if mbatch<2:
				   print(repr(lnsdelta_W1[-1,-1]))
				   print(repr(lnsdelta_W2[-1,-1]))
				   print(repr(lnsW1[-1,-1]))
				   print(repr(lnsW2[-1,-1]))
				   print('--------')
				   print(repr(lnsones[-1,-1]))
				   print(repr(lnsx[-1,-1]))
				   print(repr(lnss1[-1,-1]))
				   print(repr(lnsa1[-1,-1]))
				   print(repr(lnss2[-1,-1]))
				   print(repr(lnsa2[-1,-1]))
				   print(repr(lnsgrad_s2[-1,-1]))
				   print(repr(lnsgrad_a1[-1,-1]))
				   print(repr(lnsgrad_s1[-1,-1]))
				   print('^^^^^^^^')
		...
		plt.suptitle('FP versus LNS xlnsvValidation and Training MNIST Accuracies', fontsize=14)
		ax.legend(['FP train', 'FP validation','LNS train', 'LNS validation'])
		plt.grid(which='both', axis='both', linestyle='-.')

		plt.savefig('plotmnist_v.png')
...
if __name__ == '__main__':
	...
	parser.add_argument('--num_epoch', default = 5)     #10 20
