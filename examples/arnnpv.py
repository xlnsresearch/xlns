...
# arnnpv.py <- arnabs8xlnsnpv2024.py <- arnabs8xlnsnp2024r.py
# np LNS with variable precision (6 for weights)
###################################################
...
def main(main_params):
	...
	#XlnsExactsb = True 
	#XlnsExactdb = True
	print("testing new softmax and * instead of @ for delta, 6 for W; 6 for MAC")
	...
	if is_training:
		...
		lnsoneplrW1 = xl.xlnsnpv(np.ones(W1.shape))+lr
		rlnsoneplrW1 = 1/lnsoneplrW1
		lnsoneplrW2 = xl.xlnsnpv(np.ones(W2.shape))+lr
		...
		for epoch in range(num_epoch):
			...
			for mbatch in range(int(split / batchsize)):
				...
				#xlnsnprA
				lnss1 = xl.xlnsnpv.hstack((lnsones, lnsx)) @ lnsW1
				#lnss1 = xl.xlnsnpv(xl.xlnsnpv.hstack((lnsones, lnsx)),23) @ lnsW1
				...
				#xlnsnprB
				lnss2 = xl.xlnsnpv.hstack((lnsones, lnsa1)) @ lnsW2
				#lnss2 =  xl.xlnsnpv(        xl.xlnsnpv.hstack((lnsones, lnsa1)),23) @ lnsW2
				...
				#xlnsnprC
				lnsgrad_a1 = lnsgrad_s2 @ xl.xlnsnpv.transpose(lnsW2[1:])
				#lnsgrad_a1 = xl.xlnsnpv(         lnsgrad_s2,23) @ xl.xlnsnpv.transpose(lnsW2[1:])
				...
				lnsdelta_W2 = xl.xlnsnpv.transpose(xl.xlnsnpv.hstack((lnsones, lnsa1))) * lnsgrad_s2
				...
				lnsdelta_W1 = xl.xlnsnpv.transpose(xl.xlnsnpv.hstack((lnsones, lnsx))) * lnsgrad_s1
			...
			for mbatch in range(int(split / batchsize)):
				...
				lnscorrect_count += np.sum(np.argmax(y, axis=1) == xl.xlnsnpv.argmax(lnss2, axis=1))
		...
		plt.suptitle('FP versus LNS xlnsnpv'+str(split)+' Validation and Training MNIST Accuracies F='+str(xl.xlnsF), fontsize=14)
		ax.legend(['FP train', 'FP validation','LNS train', 'LNS validation'])
		plt.grid(which='both', axis='both', linestyle='-.')

		plt.savefig('plotmnist_npv.png')
